...
function hostmask(msg: IrcMessage): string {
...
function maskMatches(mask: string, who: string): boolean {
...
function isService(name: string): boolean { return SERVICE_RE.test(name); }
...
function detectServiceLeak(text: string): { service: string; command: string } | null {
  const t = text.trim();
...
function stripFormatting(s: string): string { return s.replace(IRC_FMT_RE, ''); }
...
function saveIgnored(list: string[]): void {
  try { localStorage.setItem(IGNORE_KEY, JSON.stringify(list)); } catch { /* ignore */ }
}
...
function saveStr(key: string, list: string[]): void {
  try { localStorage.setItem(key, JSON.stringify(list)); } catch { /* ignore */ }
}
const MUTED_KEY = 'tchatou-muted';
const HIGHLIGHT_KEY = 'tchatou-highlights';
