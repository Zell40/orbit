...
import { SERVER, isNoticeBuffer } from './store';
  ...
  const tools: Parameters<ModelContext['registerTool']>[0][] = [
    {
      ...
      execute: () => {
        ...
        return JSON.stringify(Object.values(s.buffers).map((b) => ({
          name: b.name,
          kind: b.name === SERVER ? 'console' : isNoticeBuffer(b.name) ? 'notices' : b.isChannel ? 'channel' : 'dm',
    ...
    },
    {
      ...
      execute: (a) => {
        ...
        if (!target || target === SERVER || isNoticeBuffer(target)) return 'No channel or user to send to.';
