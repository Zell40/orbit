...
function presentNicks(): { nick: string; mine: boolean }[] {
  ...
  if (buf && !buf.isChannel && buf.name && buf.name !== '$server' && !buf.name.startsWith('$notice') && buf.name.length >= 3) {
    names.add(buf.name);
  }
  return [...names]
...
export { fmtDuration, formatUserModes, groupModeDisplay, isNickModeGroup, isBanModeGroup, banTargetLabel, splitModeAndBans, modeStringWithoutBans, joinModeLabels, formatModeChange, formatModeFlagLine, loosenNoticeText, splitNoticeLines, splitActuItems, unwrapActuUrls, actuItemHeadline } from './format-text';
